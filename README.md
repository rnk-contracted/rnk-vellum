...
- **Attacks & Abilities** — weapons, spells, and abilities listed for quick rolls (weapons also remain rollable in inventory)
...
- **Numbered inventory** — Cairn-style slots `1 … N` where N is based on STR (minimum 10); empty slots stay visible; multi-slot items (chainmail, etc.) span consecutive lines; drop items onto a slot number to place them
- **Container items** — Shadowdark backpacks/sacks/etc. auto-detect as containers; **double-click** (or pencil) opens a sub-inventory with editable capacity
- **Notepad items** — **Double-click** a Notepad name to open the note editor; notes auto-save as you type
- **Dual portrait slots** — Square character portrait (128×128) and Animal Deity portrait, both clickable to browse for an image
